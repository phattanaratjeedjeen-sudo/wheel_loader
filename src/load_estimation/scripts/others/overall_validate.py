...
def main():
    ...
    # --- Create Subplots ---
    # Create a 3x2 grid of subplots, sharing X and Y axes for easier comparison
    nrows, ncols = 3, 2
    fig, axes = plt.subplots(nrows, ncols, figsize=(12, 15), sharex=True, sharey=True)
    ...
    for i in range(len(filename)):
        ...
        # --- Calculate overall STD for the entire static dataset of the file ---
        overall_std_w = static_df[col_w].std()
        ...
        errorbar_container = ax.errorbar(bin_centers, avg_w_in_bin, yerr=overall_std_w,
        ...
        # Write the value of the actual load line on the plot
        ax.text(0.98, ACTUAL_LOAD[i], f' {ACTUAL_LOAD[i]:.0f} kg',
                transform=ax.get_yaxis_transform(),
                ha='right', va='bottom', color=series_color, fontsize=9,
                bbox=dict(facecolor='white', alpha=0.7, edgecolor='none', boxstyle='round,pad=0.2'))
        ax.set_title(f'Experiment: {filename[i]}')
        ax.legend()
        ax.grid(True)
    
    # --- 5. Finalize and show plot ---
    # Hide any unused subplots
    for i in range(len(filename), len(axes)):
        axes[i].set_visible(False)

    fig.suptitle('Estimated Load (w) vs. Boom Angle (theta_g)', fontsize=16)
    fig.supxlabel('theta_g (rad)')
    fig.supylabel('load w (kg)')
